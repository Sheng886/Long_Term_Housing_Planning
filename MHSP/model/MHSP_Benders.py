...
class subporblem():
    def __init__(self, args, input_data):
        ...
        # Scen_Path variable
        self.vk = self.sub.addVars(args.T+1, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='vnktwp')
        self.bk = self.sub.addVars(args.T+1, args.I, lb=0.0, vtype=GRB.CONTINUOUS, name='bnkti')
        self.ak = self.sub.addVars(args.T+1, args.I, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='anktiwp')
        self.fk = self.sub.addVars(args.T+1, args.W, args.J, args.P, args.G, lb=0.0, vtype=GRB.CONTINUOUS, name='fnktwjpg')
        self.sk = self.sub.addVars(args.T+1, args.J, args.G, lb=0.0, vtype=GRB.CONTINUOUS, name='snktjg')
        ...
        self.sub.setObjective(quicksum(self.idata.O_p[p]*self.aak[w,p] - self.idata.R_p[p]*self.idata.O_p[p]*self.bbk[w,p] for w in range(args.W) for p in range(args.P)) 
                              ...
                              + quicksum(self.idata.CU_g[g]*self.sk[t,j,g] for j in range(args.J) for g in range(args.G)) for t in range(args.T+1)), GRB.MINIMIZE);
        ...
        # Dual
        self.i_Production_Capacity_cons = [[0 for i in range(args.I)] for t in range(args.T+1)]
        ...
        # receive self.u[n,w]
        # Dual
        self.k_Staging_Capacity_cons = [[0 for w in range(args.W)] for t in range(args.T+1)]
        for t in range(args.T+1):
            for w in range(args.W):
        ...
        # Dual
        self.l_Demand_Flow_cons = [[[0 for g in range(args.G)] for j in range(args.J)] for t in range(args.T+1)]
        for t in range(1,args.T+1):
        ...
        for w in range(args.W):
            for p in range(args.P):
                self.m_Replensih_cons[w][p] = self.sub.addConstr(self.vk[args.T,w,p] + self.aak[w,p] + self.bbk[w,p] == 0)
    ...
    def run(self,args,n,k,v_vals,u_vals):
        ...
        for t in range(1,args.T+1):
            for j in range(args.J):
                for g in range(args.G):
                    self.l_Demand_Flow_cons[t][j][g].setAttr(GRB.Attr.RHS, self.tree[n].children_blackpath[k].demand[g][t-1]*self.idata.J_pro[j])
        ...
        pi_i = np.zeros((args.T+1, args.I))
        pi_k = np.zeros((args.T+1, args.W))
        pi_l = np.zeros((args.T+1,args.J,args.G))
        ...
        for t in range(1,args.T+1):
            for j in range(args.J):
                for g in range(args.G):
                    ...
                    temp = temp + pi_l[t][j][g]*self.tree[n].children_blackpath[k].demand[g][t-1]*self.idata.J_pro[j]
...
class Benders():
    def __init__(self, args, input_data, subporblem):
        ...
        # Stage variable
        self.u = self.master.addVars(args.n, args.W, lb=0.0, vtype=GRB.CONTINUOUS, name='unw')
        self.y = self.master.addVars(args.n, args.W, lb=0.0, vtype=GRB.CONTINUOUS, name='ynw')
        self.v = self.master.addVars(args.n, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='vnwp')
        self.x = self.master.addVars(args.n, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='xnwp')
        self.z = self.master.addVars(args.n, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='znwp')
        self.theta = self.master.addVars(args.n, args.K, lb=0.0, vtype=GRB.CONTINUOUS, name='thetanwp')
        ...
        self.master.setObjective(quicksum(self.tree[n].prob_to_node*(quicksum(self.idata.E_w[w]*self.y[n,w] for w in range(args.W)) 
                                                                  ...
                                                                  + quicksum(self.tree[n].prob2Children_black[k]*self.theta[n,k] for k in range(args.K))) for n in range(args.n)), GRB.MINIMIZE);
    ...
    def run(self,args):
        ...
        while(abs((self.UB - self.LB)/self.UB) >= self.eps):
                ...
                for k in range(args.K):
                    ...
                    if(self.theta[n,k].x < obj[n][k] - cut_vio_thred and abs(self.theta[n,k].x - obj[n][k])/max(abs(self.theta[n,k].x),1e-10) > cut_vio_thred):
                        ...
                        self.master.addConstr(self.theta[n,k] >= quicksum(self.v[n,w,p]*pi_f[w][p] for w in range(args.W) for p in range(args.P)) 
                                                                + quicksum(self.idata.B_i[i]*pi_i[t][i] for t in range(args.T+1) for i in range(args.I))
                                                                + quicksum(self.u[n,w]*pi_k[t][w] for t in range(args.T+1) for w in range(args.W))
                                                                + quicksum(self.tree[n].children_blackpath[k].demand[g][t-1]*self.idata.J_pro[j]*pi_l[t][j][g] for t in range(1,args.T+1) for j in range(args.J) for g in range(args.G))
            ...
            UB_temp = sum(self.tree[n].prob_to_node*(sum(self.idata.E_w[w]*self.y[n,w].x for w in range(args.W)) 
                                                   ...
                                                   + sum(self.tree[n].prob2Children_black[k]*obj[n][k] for k in range(args.K))) for n in range(args.n)) 
