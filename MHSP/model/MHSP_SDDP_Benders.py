...
class CutPool:
    def __init__(self):
        ...
        self.pi_8b_np = np.array([])
        self.pi_8e_np = np.array([])
        self.pi_8g_np = np.array([])
        self.pi_8h_np = np.array([])
        self.pi_8i_np = np.array([])

    def add_cut(self, pi_8b, pi_8e, pi_8g, pi_8h, pi_8i):
        # Append new elements to each array and update the instance attributes
        self.pi_8b_np = np.append(self.pi_8b_np, pi_8b)
        self.pi_8e_np = np.append(self.pi_8e_np, pi_8e)
        self.pi_8g_np = np.append(self.pi_8g_np, pi_8g)
        self.pi_8h_np = np.append(self.pi_8h_np, pi_8h)
        self.pi_8i_np = np.append(self.pi_8i_np, pi_8i)
...
class solve_SDDP:
    def __init__(self, args, input_data):
        ...
        if(args.Strategic_node_sovling == 1):
            ...
            self.cutpool_state = [CutPool() for n in range(args.N)]
    ...
    def run(self):
        ...
        for iter in range(self.args.MAX_ITER):
            ...
            # ---------------------------------------------------- Forward ----------------------------------------------------
            if(self.args.Strategic_node_sovling == 1):
