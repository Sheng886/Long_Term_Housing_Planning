...
class StageProblem:
    ...
    def backward_run(self):
        ...
        if(self.phi.x < sub_opt_total - cut_vio_thred and abs(self.phi.x - sub_opt_total)/max(abs(self.phi.x),1e-10) > cut_vio_thred):
            ...
            print("stage:",self.stage,"state:",self.state,"add cut")




        return pi_b,pi_c,self.model.ObjVal

    def add_cut(self,obj,stage_next,state_sample_path,state_next,pi_b,pi_c):
...
class solve_SDDP:
    ...
    def run(self):
        ...
        for iter in range(self.args.MAX_ITER):
            ...
            # ----------------------------------- Backward -----------------------------------
            pi_b,pi_c,LB = self.stage_leaf[sample_path[self.args.T-1]].backward_run()
            for stage in reversed(range(self.args.T-1)):

                # ---------------------------- Cut Sharing ----------------------------
                for state in range(self.args.N):
                    cut_iter_temp = self.stage[stage][state].add_cut(LB,stage+1,sample_path[stage],sample_path[stage+1],pi_b,pi_c)
                    cutviol_iter = cutviol_iter + cut_iter_temp

                pi_b,pi_c,LB =  self.stage[stage][sample_path[stage]].backward_run()

            cut_iter_temp = self.stage_root.add_cut(LB,0,0,sample_path[0],pi_b,pi_c)
            cutviol_iter = cutviol_iter + cut_iter_temp

            pi_b,pi_c,LB =  self.stage_root.backward_run()
