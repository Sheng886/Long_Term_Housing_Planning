...
class baseline_class():
    def __init__(self, args, input_data):
        ...
        # Scen_Path variable
        self.vk = self.model.addVars(args.n, args.K, args.T+1, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='vnktwp')
        self.bk = self.model.addVars(args.n, args.K, args.T+1, args.I, lb=0.0, vtype=GRB.CONTINUOUS, name='bnkti')
        self.ak = self.model.addVars(args.n, args.K, args.T+1, args.I, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='anktiwp')
        self.fk = self.model.addVars(args.n, args.K, args.T+1, args.W, args.J, args.P, args.G, lb=0.0, vtype=GRB.CONTINUOUS, name='fnktwjpg')
        self.sk = self.model.addVars(args.n, args.K, args.T+1, args.J, args.G, lb=0.0, vtype=GRB.CONTINUOUS, name='snktjg')
        self.aak = self.model.addVars(args.n, args.K, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='aanktwp')
        self.bbk = self.model.addVars(args.n, args.K, args.W, args.P, lb=0.0, vtype=GRB.CONTINUOUS, name='bbnktwp')
                ...
                for t in range(1,args.T+1):
                    for j in range(args.J):
                        for g in range(args.G):
                            self.model.addConstr(quicksum(self.fk[n,k,t,w,j,p,g] for w in range(args.W) for p in range(args.P)) + self.sk[n,k,t,j,g] == self.tree[n].children_blackpath[k].demand[g][t-1]*self.idata.J_pro[j])
