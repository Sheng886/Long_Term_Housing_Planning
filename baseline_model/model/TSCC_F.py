    ...
    def __init__(self, args):
        ...
        for i in range(args.I):
            for p in range(args.P):
                for t in range(self.first_end_time_point,args.T-1):
                    self.model.addConstr(self.vk[k,i,p,t+1] + sum(self.fk_ij[k,i,j,p,t] for j in range(args.J)) == self.vk[k,i,p,t] + self.nuk[k,i,p,t-self.low_supplier_prod_time[i][p]])
        ...
        for w in range(args.W):
            for p in range(args.P):
                for t in range(self.first_end_time_point,args.T-1):
                    for k in range(args.K):
                        self.model.addConstr(self.mk[k,w,p,t+1] + sum(self.fk_wj[k,w,j,p,t] for j in range(args.J)) == self.mk[k,w,p,t])
        ...
        for k in range(args.K):
            for i in range(args.I):
                for t in range(self.first_end_time_point,args.T):
                    self.model.addConstr(self.bk[k,i,t] <= self.num_production_line[i] + self.z[k]*sum(self.house_0_match[p]*self.demand[k][g][j] for g in range(args.G) for j in range(args.J)))
