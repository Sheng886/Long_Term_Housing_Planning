...
class two_stage_chance():
    ...
    def check_MIPSOL(self,model,where):
        if where == GRB.Callback.MIPSOL:
            ...
            pi_2h = np.zeros((self.args.K,self.args.J,self.args.G))
            ...
            for k in range(self.args.K):
                sub.setObjective(sum(self.area_region_distance[w][j]*self.house_volumn_factor[p]*self.trans_price*fk_wj[w,j,p,t] for w in range(self.args.W) for j in range(self.args.J) for p in range(self.args.P) for t in range(self.first_end_time_point,self.args.T))
                                + sum(self.unmet[g]*self.house_price[g+1]*(self.demand[k][g][j] - dk[j,g,self.args.T-1]) for j in range(self.args.J) for g in range(self.args.G))
                                ...
                                + sum((self.deprivation_a0[0] + self.deprivation_a1[0]*(self.args.T-t))*self.house_price[g+1]*(self.demand[k][g][j]-dk[j,g,t]) for j in range(self.args.J) for t in range(self.deprivation_time_point,self.args.T) for g in range(self.args.G))
                ...
                for j in range(self.args.J):
                    for g in range(self.args.G):
                        temp = sub.addConstr(dk[j,g,self.args.T-1] <= self.demand[k][g][j])
                ...
                for j in range(self.args.J):
                    for g in range(self.args.G):
                        pi_2h[k][j][g] = demand_cons[self.args.G*j+g].pi
                        sol_temp = sol_temp + pi_2h[k][j][g]*self.demand[k][g][j]
                ...
                const1[k] = sum((self.deprivation_a0[0] + self.deprivation_a1[0]*(self.args.T-t))*self.house_price[g+1]*(self.demand[k][g][j]) for j in range(self.args.J) for t in range(self.deprivation_time_point,self.args.T) for g in range(self.args.G))
                const2[k] = sum(self.unmet[g]*self.house_price[g+1]*(self.demand[k][g][j]) for j in range(self.args.J) for g in range(self.args.G))
                ...
                M1[k] = M1[k] + const1[k] + const2[k]
                M2[k] = M2[k] + const1[k] + const2[k]
                ...
                # import pdb;pdb.set_trace()
                # print("dual:",sol_temp + const1[k] + const2[k])
            ...
            model.cbLazy(model.getVars()[-1] + sum(M1[k]*model._zvars[k] for k in range(self.args.K) if z_vals[k] == 0) + sum(M2[k]*(1-model._zvars[k]) for k in range(self.args.K) if z_vals[k] == 1) 
                    >= (1/self.args.K)*(sum(model._mvars[w,p,self.first_end_time_point]*pi_2b[k][w][p] for w in range(self.args.W) for p in range(self.args.P) for k in range(self.args.K))
                                       ...
                                       +sum(pi_2h[k][j][g]*self.demand[k][g][j] for j in range(self.args.J) for g in range(self.args.G) for k in range(self.args.K))
                                       ...
                                       +sum(const1[k] + const2[k] for k in range(self.args.K))))
