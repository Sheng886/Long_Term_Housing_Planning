...
class input_data:
    def __init__(self, args):
        ...
        first_end_time_point_TIU = int(math.ceil(self.first_end_time_point/args.TIU_pre))
        second_end_time_point_TIU = first_end_time_point_TIU + int(math.ceil((self.second_end_time_point-self.first_end_time_point)/args.TIU_post))
        deprivation_time_point_TIU = first_end_time_point_TIU + int(math.ceil((self.deprivation_time_point-self.first_end_time_point)/args.TIU_post))
        ...
        for i in range(0,args.P):
            ...
            self.house_install_time[i] = int(math.ceil(df[self.house_name[i]][3]/args.TIU_post))
        ...
        self.deprivation_a0 = df["a_0"]*args.TIU_post
        self.deprivation_a1 = df["a_1"]*args.TIU_post
        ...
        for i in range(0,args.W):
            self.staging_area_capacity[i] = df[staging_area_column[2]][i]*args.TIU_post
            self.high_staging_area_flow[i] = df[staging_area_column[3]][i]*args.TIU_post
            self.mid_staging_area_flow[i] = df[staging_area_column[4]][i]*args.TIU_post
            self.low_staging_area_flow[i] = df[staging_area_column[5]][i]*args.TIU_post
        ...
        for i in range(0,args.I):
            ...
            self.high_supplier_flow[i] = df[supplier_column[26]][i]*args.TIU_post
            self.mid_supplier_flow[i] = df[supplier_column[27]][i]*args.TIU_pre
            self.low_supplier_flow[i] = df[supplier_column[28]][i]*args.TIU_pre
            for p in range(0, args.P):
                ...
                self.high_supplier_prod_time[i][p] = int(math.ceil(df[supplier_column[15+p]][i]/args.TIU_post))
                self.mid_supplier_prod_time_TIU[i][p] = int(math.ceil(df[supplier_column[18+p]][i]/args.TIU_pre))
                self.low_supplier_prod_time_TIU[i][p] = int(math.ceil(df[supplier_column[21+p]][i]/args.TIU_pre))
...
class sub_N():
    def __init__(self,args,input_data):
        ...
        self.sub_Normal.setObjective(quicksum(self.input_data.area_region_distance[w][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price*fk_wj[w,j,p,t] for w in range(args.W) for j in range(args.J) for p in range(args.P) for t in range(self.input_data.first_end_time_point,args.T))
                ...
                + quicksum((self.input_data.mismatch[p-1][g])*self.input_data.house_price[p]*yk[j,p,g,t] for j in range(args.J) for p in range(1,args.P) for g in range(args.G) for t in range(self.input_data.first_end_time_point,args.T))
                ...
                + quicksum((self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU_post)*(args.T-t))*(self.input_data.house_price[g+1])*(qk[j,g,t]) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
                + quicksum((args.emergency_price_factor*self.input_data.house_price[p] + self.input_data.supplier_region_distance[i][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price)*fk_ij[i,j,p,t] for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T) for p in range(args.P))
                ,GRB.MINIMIZE);
        ...
        for j in range(args.J):
            for p in range(args.P):
                for t in range(0,self.input_data.first_end_time_point + int(self.input_data.house_install_time[0])):
        ...
        for j in range(args.J):
            for p in range(args.P):
                for g in range(args.G):
                    self.sub_Normal.addConstr(yk[j,p,g,args.T-1] == 0)
                    for t in range(0,self.input_data.first_end_time_point):
        ...
        for w in range(args.W):
                for p in range(args.P):
                    for t in range(self.input_data.first_end_time_point,args.T-1):
                        temp = self.sub_Normal.addConstr(mk[w,p,t+1] + quicksum(fk_wj[w,j,p,t] for j in range(args.J)) - mk[w,p,t] == 0)
        ...
        for j in range(args.J):
            for t in range(self.input_data.first_end_time_point,args.T):
                for p in range(1,args.P):
                    self.sub_Normal.addConstr(1/self.input_data.house_0_match[p]*fk0_j[j,p,t] + quicksum(fk_wj[w,j,p,t] for w in range(args.W)) + quicksum(fk_ij[i,j,p,t] for i in range(args.I)) == quicksum(yk[j,p,g,t] for g in range(args.G))) 

        # demand flow
        for j in range(args.J):
            for t in range(args.T-1):
                for g in range(args.G):
                    self.sub_Normal.addConstr(dk[j,g,t+1] == dk[j,g,t] + quicksum(yk[j,p,g,t] for p in range(1,args.P)))
    ...
    def run(self,args,k,x_vals,m_vals,v_vals):
        ...
        for w in range(args.W):
            for p in range(args.P):
                for t in range(args.T-self.input_data.first_end_time_point-1):
                    if(abs(sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for i in range(args.I))) < nonint_ratio):
                        ...
                        self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].setAttr(GRB.Attr.RHS,sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for i in range(args.I)))
        for j in range(args.J):
            for g in range(args.G):
        ...
        for w in range(args.W):
            for p in range(args.P):
                for t in range(args.T-self.input_data.first_end_time_point-1):
                    ...
                    # sol_temp = sol_temp + pi_2c[w][p][t]*sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for i in range(args.I))
        ...
        #     print("Primal and Dual are not match")
        #     print("dula:",sol_temp)
    ...
    def run_evaluation(self,args,k,x_vals,m_vals,v_vals,new_dir_path):
        ...
        for i in range(args.I):
            for p in range(args.P):
                if(abs(v_vals[i,p,self.input_data.first_end_time_point].x) < nonint_ratio):
                    ...
                    # self.S_initial_inventory_cons[i*args.P+p].rhs = v_vals[i,p,self.input_data.first_end_time_point].x
        ...
        for w in range(args.W):
            for p in range(args.P):
                if(abs(m_vals[w,p,self.input_data.first_end_time_point].x) < nonint_ratio):
                    ...
                    # self.W_initial_inventory_cons[w*args.P+p].rhs = m_vals[w,p,self.input_data.first_end_time_point].x
                    self.W_initial_inventory_cons[w*args.P+p].setAttr(GRB.Attr.RHS, m_vals[w,p,self.input_data.first_end_time_point].x)

        for w in range(args.W):
            for p in range(args.P):
                for t in range(args.T-self.input_data.first_end_time_point-1):
                    if(abs(sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for i in range(args.I))) < nonint_ratio):
                        # self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].rhs = 0
                        self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].setAttr(GRB.Attr.RHS, 0)
                    else:
                        # self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].rhs = sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for i in range(args.I))
                        self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].setAttr(GRB.Attr.RHS, sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for i in range(args.I)))
        ...
        if self.sub_Normal.status == GRB.OPTIMAL:
            ...
            print(k, k, "Normal infesible or unbounded")
            pdb.set_trace()
        ...
        deprivation = sum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU_post)*(args.T-t))*self.input_data.house_price[g+1]*(self.qk[j,g,t].x) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
...
class sub_R():
    def __init__(self,args,input_data):
        ...
        for j in range(args.J):
            for p in range(args.P):
                for g in range(args.G):
                    self.sub_Recovery.addConstr(yk[j,p,g,args.T-1] == 0)
                    for t in range(0,self.input_data.first_end_time_point):
        ...
        for w in range(args.W):
                for p in range(args.P):
                    for t in range(self.input_data.first_end_time_point,args.T-1):
                        temp = self.sub_Recovery.addConstr(mk[w,p,t+1] + quicksum(fk_wj[w,j,p,t] for j in range(args.J)) - mk[w,p,t] == 0)
                        self.W_inventory_flow_cons.append(temp)
        ...
        for j in range(args.J):
            for t in range(self.input_data.first_end_time_point,args.T):
                for p in range(1,args.P):
                    self.sub_Recovery.addConstr(1/self.input_data.house_0_match[p]*fk0_j[j,p,t] + quicksum(fk_wj[w,j,p,t] for w in range(args.W)) + quicksum(fk_ij[i,j,p,t] for i in range(args.I)) == quicksum(yk[j,p,g,t] for g in range(args.G))) 

        # demand flow
        for j in range(args.J):
            for t in range(args.T-1):
                for g in range(args.G):
                    self.sub_Recovery.addConstr(dk[j,g,t+1] == dk[j,g,t] + quicksum(yk[j,p,g,t] for p in range(1,args.P)))
        ...
        for i in range(args.I):
            for t in range(self.input_data.first_end_time_point,args.T-1):
                    temp = self.sub_Recovery.addConstr(bk[i,t] + quicksum(nuk[i,p,t-self.input_data.low_supplier_prod_time_TIU[i][p]] for p in range(args.P)) == quicksum(nuk[i,p,t] for p in range(args.P)) + bk[i,t+1]) 
        ...
        for i in range(args.I):
            for p in range(args.P):
                for t in range(self.input_data.first_end_time_point,args.T-1):
                    temp = self.sub_Recovery.addConstr(vk[i,p,t+1]  + quicksum(fk_ij[i,j,p,t] for j in range(args.J)) - vk[i,p,t] - (1/self.input_data.low_supplier_prod_time[i][p])*(self.input_data.low_supplier_prod_time_TIU[i][p])*args.TIU_post*nuk[i,p,t-self.input_data.low_supplier_prod_time_TIU[i][p]] == 0)
        ...
        for j in range(args.J):
            for t in range(args.T):
                temp = self.sub_Recovery.addConstr(quicksum(fk_ij[i,j,0,t] for i in range(args.I)) + quicksum(fk_wj[w,j,0,t] for w in range(args.W)) - quicksum(fk0_j[j,p,t+self.input_data.house_install_time[0]] for p in range(1,args.P) if t + self.input_data.house_install_time[0] <= args.T-1) == 0) 
                self.novel_house_match_cons.append(temp)
        ...
        for i in range(args.I):
            for p in range(args.P):
                for t in range(args.T-1-self.input_data.first_end_time_point):
                    if(abs((1/self.input_data.low_supplier_prod_time[i][p])*(self.input_data.low_supplier_prod_time_TIU[i][p])*args.TIU_post*s_vals[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]+self.input_data.first_end_time_point] - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W))) < nonint_ratio):
                    ...
                    # self.supplier_inventory_cons[i*args.P*(args.T-1-self.input_data.first_end_time_point)+p*(args.T-1-self.input_data.first_end_time_point)+t].rhs = s_vals[i,p,t-self.input_data.mid_supplier_prod_time[i][p]+self.input_data.first_end_time_point] - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W))
                        self.supplier_inventory_cons[i*args.P*(args.T-1-self.input_data.first_end_time_point)+p*(args.T-1-self.input_data.first_end_time_point)+t].setAttr(GRB.Attr.RHS, (1/self.input_data.low_supplier_prod_time[i][p])*(self.input_data.low_supplier_prod_time_TIU[i][p])*args.TIU_post*s_vals[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]+self.input_data.first_end_time_point] - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W)))
        ...
        for i in range(args.I):
            for p in range(args.P):
                for t in range(args.T-1-self.input_data.first_end_time_point):
                    ...
                    # sol_temp = sol_temp + (s_vals[i,p,t-self.input_data.mid_supplier_prod_time[i][p]+self.input_data.first_end_time_point] - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W)))*pi_4d[i][p][t]
    ...
    def run_evaluation(self,args,k,s_vals,x_vals,m_vals,v_vals,new_dir_path):
        ...
        for i in range(args.I):
            for p in range(args.P):
                for t in range(args.T-1-self.input_data.first_end_time_point):
                    if(abs((1/self.input_data.low_supplier_prod_time[i][p])*(self.input_data.low_supplier_prod_time_TIU[i][p])*args.TIU_post*s_vals[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]+self.input_data.first_end_time_point].x - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for w in range(args.W))) < nonint_ratio):
                        self.supplier_inventory_cons[i*args.P*(args.T-1-self.input_data.first_end_time_point)+p*(args.T-1-self.input_data.first_end_time_point)+t].setAttr(GRB.Attr.RHS, 0)
                    else:
                    # self.supplier_inventory_cons[i*args.P*(args.T-1-self.input_data.first_end_time_point)+p*(args.T-1-self.input_data.first_end_time_point)+t].rhs = s_vals[i,p,t-self.input_data.mid_supplier_prod_time[i][p]+self.input_data.first_end_time_point] - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W))
                        self.supplier_inventory_cons[i*args.P*(args.T-1-self.input_data.first_end_time_point)+p*(args.T-1-self.input_data.first_end_time_point)+t].setAttr(GRB.Attr.RHS, (1/self.input_data.low_supplier_prod_time[i][p])*(self.input_data.low_supplier_prod_time_TIU[i][p])*args.TIU_post*s_vals[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]+self.input_data.first_end_time_point].x - sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for w in range(args.W)))
        ...
        if self.sub_Recovery.status == GRB.OPTIMAL:
            ...
            print(k, k, "Recovery infesible or unbounded")
        ...
        return self.sub_Recovery.ObjVal,logistc_cost,unmet,mismatch,unused,deprivation,emergency_cost,unmet1,unmet2,unused1,unused2,unused3

class master():
    def __init__(self,args,input_data,sub_N,sub_R):
        ...
        for i in range(args.I):
            ...
            for t in range(args.T-1):
                self.model.addConstr(self.a[i,t+1] + quicksum(self.s[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]] for p in range(args.P) if t-self.input_data.mid_supplier_prod_time_TIU[i][p] >= 0) ==  quicksum(self.s[i,p,t] for p in range(args.P)) + self.a[i,t])
            ...
            for p in range(args.P):
                ...
                for t in range(args.T-1):
                    if(t-self.input_data.mid_supplier_prod_time_TIU[i][p] >=0):
                        self.model.addConstr(self.v[i,p,t+1] + quicksum(self.x[i,w,p,t] for w in range(args.W)) ==  self.v[i,p,t] + (1/self.input_data.mid_supplier_prod_time[i][p])*(self.input_data.mid_supplier_prod_time_TIU[i][p])*args.TIU_post*self.s[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]])
                    else:
                        self.model.addConstr(self.v[i,p,t+1] + quicksum(self.x[i,w,p,t] for w in range(args.W)) ==  self.v[i,p,t] )                    
        ...
        for w in range(args.W): 
            for t in range(args.T-1):
                for p in range(args.P):
                    self.model.addConstr(self.m[w,p,t+1] == quicksum(self.x[i,w,p,t] for i in range(args.I)) + self.m[w,p,t])
                ...
                if(theta_vals < lower_bound - cut_vio_thred and abs(theta_vals - lower_bound)/max(abs(theta_vals),1e-10) > cut_vio_thred):
                    ...
                    model.cbLazy(model.getVars()[-1] + quicksum(M1[k]*model._zvars[k] for k in range(args.K) if abs(z_vals[k]) < nonint_ratio) + quicksum(M2[k]*(1-model._zvars[k]) for k in range(args.K) if abs(z_vals[k]-1) < nonint_ratio) 
                        >= (1/args.K)*(quicksum(quicksum(pi_2h[0][k][j][g][t]*self.input_data.demand[k][g][j] for g in range(args.G) for j in range(args.J) for t in range(args.T-self.input_data.first_end_time_point) if(zvars_round[k] == 0))
                                                    ...
                                                    + quicksum(pi_2c[0][k][w][p][t]*quicksum(model._xvars[i,w,p,t+self.input_data.first_end_time_point] for i in range(args.I)) for w in range(args.W) for p in range(args.P) for t in range(args.T-self.input_data.first_end_time_point-1) if(zvars_round[k] == 0))
                                                    ...
                                                    + quicksum(pi_2c[1][k][w][p][t]*quicksum(model._xvars[i,w,p,t+self.input_data.first_end_time_point] for i in range(args.I)) for w in range(args.W) for p in range(args.P) for t in range(args.T-self.input_data.first_end_time_point-1) if(zvars_round[k] == 1))
                                                    ...
                                                    + quicksum((model._svars[i,p,t-self.input_data.mid_supplier_prod_time_TIU[i][p]+self.input_data.first_end_time_point] - quicksum(model._xvars[i,w,p,t+self.input_data.first_end_time_point] for w in range(args.W)))*pi_4d[1][k][i][p][t] for i in range(args.I) for p in range(args.P) for t in range(args.T-1-self.input_data.first_end_time_point) if(zvars_round[k] == 1))
    ...
    def run_eval(self,args,s,x,m,v):
        ...
        new_dir_path =  "result/" + str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIU_" + str(args.TIU) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10))
        ...
        for k in range(args.K):
            ...
            second_obj[1][k],logistc_cost[1][k],unmet[1][k],mismatch[1][k],unused[1][k],deprivation[1][k],emergency_cost[1][k],unmet1[1][k],unmet2[1][k],unused1[1][k],unused2[1][k],unused3[1][k] = self.sub_R.run_evaluation(args,k,s,x,m,v,new_dir_path)
        ...
        for i in range(len(temp_zk)):
            index.append(temp_zk.index(sorted(temp_zk,reverse=True)[i]))
        ...
        for k in range(args.K):
            ...
            print(k,second_obj[temp][index[k]])
        ...
        df_name = ["back up active","OPT","V_star","first_stage_trans_cost","type0","type1","type2",
                   "second_stage_cots","second_trans_cost","Emergency_cost","unused_cost","unused0","unused1","unused2","mismatch_cost","unmet_cost","shortage 1","shortage 2","deprivation"]
        data = [[e_eval, 
        ...
        name = "result/" + str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIUUU_" + str(args.TIU_post) + "_" + str(args.TIU_pre) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + ".csv"
        df.to_csv(name)
        ...
        if(args.Large_Scenario_main == 1):
            ...
            f_generate.write("\nopt: %s" % first_stage_cost+second_obj_total/args.K)
    ...
    def run(self,args):
        ...
        result_name = "result/" + str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIUUU_" + str(args.TIU_pre) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + ".txt"
        f = open(result_name,"w+")
