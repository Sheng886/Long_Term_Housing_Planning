...
class input_data:
    def __init__(self, args, TIU):
        ...
        self.first_end_time_point = int(df["First_End"][0])
        self.deprivation_time_point = int(df["Deprivation_Start"][0])
        self.second_end_time_point = int(df["Second_End"][0])
...
class master():
    ...
    def run_eval(self,args,s,x,m,v):
        ...
        name = args.result_path + "/" + "TIUEVAL" +str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIU_" + str(self.input_data.TIU) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + "_mis_" + str(int(args.mis_factor*10)) + "_un_" + str(int(args.unused_factor*10)) + ".csv"
        df.to_csv(name)
    ...
    def run(self,args,z,x,s,m,v):
        ...
        print(self.input_data.first_end_time_point,self.input_data.deprivation_time_point,args.T)

        for k in range(args.K):
            if (k < args.error):
                self.model.addConstr(self.z[k] == z[k].x)

        
        if(args.TIU_eq != 1):
          temp_ = int(args.T/self.input_data.TIU)
          for i in range(args.I):
              for w in range(args.W):
                  for p in range(args.P):
                      for t in range(temp_):
                          self.model.addConstr(quicksum(self.x[i,w,p,t*self.input_data.TIU+tt] for tt in range(self.input_data.TIU)) <= x[i,w,p,t].x)
        else:
          temp_ = int(args.T/self.input_data.TIU)
          for i in range(args.I):
              for w in range(args.W):
        ...
        # for i in range(args.I):
        #     for p in range(args.P):
        #         for t in range(0,args.T,self.input_data.TIU):
        #             self.model.addConstr(quicksum(self.s[i,p,t+tt] for tt in range(self.input_data.TIU)) <= s[i,p,int(t/self.input_data.TIU)].x)


        # for i in range(args.I):
        #     for p in range(args.P):
        #         for t in range(0,args.T,self.input_data.TIU):
        #             self.model.addConstr(quicksum(self.v[i,p,t+tt] for tt in range(self.input_data.TIU)) <= v[i,p,int(t/self.input_data.TIU)].x)


        self.model.optimize(lambda model, where: self.check_MIPSOL(args,model, where))
   

        result_name = args.result_path + "/" + "TIUEVAL"+ str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIU_" + str(self.input_data.TIU) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + "_mis_" + str(int(args.mis_factor*10)) + "_un_" + str(int(args.unused_factor*10)) + "_Heu_" + str(args.Heu) + "_cut_save_" + str(args.cut_save) + ".txt"
