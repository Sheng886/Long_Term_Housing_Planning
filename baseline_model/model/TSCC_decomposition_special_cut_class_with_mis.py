    ...
    def __init__(self,args,input_data):
            ...
            for t in range(self.input_data.first_end_time_point,args.T):
                for p in range(1,args.P):
                    if(t - self.input_data.house_install_time[0] >= self.input_data.first_end_time_point + 1):
                        self.sub_Normal.addConstr(1/self.input_data.house_0_match[p]*fk0_j[j,p,t-self.input_data.house_install_time[0]] + quicksum(fk_wj[w,j,p,t-self.input_data.wj_t[w][j]] for w in range(args.W) if (t-self.input_data.wj_t[w][j] >= self.input_data.first_end_time_point + 1)) + quicksum(fk_ij[i,j,p,t] for i in range(args.I)) == quicksum(yk[j,p,g,t] for g in range(args.G))) 
                    else:
                        self.sub_Normal.addConstr(quicksum(fk_wj[w,j,p,t-self.input_data.wj_t[w][j]] for w in range(args.W) if (t-self.input_data.wj_t[w][j] >= self.input_data.first_end_time_point + 1)) + quicksum(fk_ij[i,j,p,t] for i in range(args.I)) == quicksum(yk[j,p,g,t] for g in range(args.G))) 
        ...
        for j in range(args.J):
            for t in range(args.T-1):
                for g in range(args.G):
                    self.sub_Normal.addConstr(dk[j,g,t+1] == dk[j,g,t] + quicksum(yk[j,p,g,t+1] for p in range(1,args.P)))
            ...
            for p in range(args.P):
                for t in range(args.T-self.input_data.first_end_time_point-1):
                    if(abs(sum(x_vals[i,w,p,t+self.input_data.first_end_time_point].x for i in range(args.I))) < nonint_ratio):
                        ...
                        self.W_inventory_flow_cons[w*args.P*(args.T-self.input_data.first_end_time_point-1)+p*(args.T-self.input_data.first_end_time_point-1)+t].setAttr(GRB.Attr.RHS, sum(x_vals[i,w,p,t+self.input_data.first_end_time_point-self.input_data.iw_t[i][w]].x for i in range(args.I) if (t+self.input_data.first_end_time_point-self.input_data.iw_t[i][w]) >= 0))
...
class sub_R():
    def __init__(self,args,input_data):
        ...
        for j in range(args.J):
            for t in range(self.input_data.first_end_time_point,args.T):
                for p in range(1,args.P):
                    if(t - self.input_data.house_install_time[0] >= self.input_data.first_end_time_point + 1):
                        self.sub_Recovery.addConstr(1/self.input_data.house_0_match[p]*fk0_j[j,p,t-self.input_data.house_install_time[0]] + quicksum(fk_wj[w,j,p,t-self.input_data.wj_t[w][j]] for w in range(args.W) if t-self.input_data.wj_t[w][j] >= self.input_data.first_end_time_point+1) + quicksum(fk_ij[i,j,p,t-self.input_data.ij_t[i][j]] for i in range(args.I) if t-self.input_data.ij_t[i][j]>= self.input_data.first_end_time_point+1) == quicksum(yk[j,p,g,t] for g in range(args.G))) 
                    else:
                       self.sub_Recovery.addConstr(quicksum(fk_wj[w,j,p,t-self.input_data.wj_t[w][j]] for w in range(args.W) if t-self.input_data.wj_t[w][j] >= self.input_data.first_end_time_point+1) + quicksum(fk_ij[i,j,p,t-self.input_data.ij_t[i][j]] for i in range(args.I) if t-self.input_data.ij_t[i][j]>= self.input_data.first_end_time_point+1) == quicksum(yk[j,p,g,t] for g in range(args.G))) 
        ...
        for j in range(args.J):
            for t in range(args.T-1):
                for g in range(args.G):
                    self.sub_Recovery.addConstr(dk[j,g,t+1] == dk[j,g,t] + quicksum(yk[j,p,g,t+1] for p in range(1,args.P)))
    ...
    def run_evaluation(self,args,k,s_vals,x_vals,m_vals,v_vals,new_dir_path):
        ...
        deprivation = sum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*self.input_data.house_price[g+1]*(self.qk[j,g,t].x) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
        emergency_cost = sum((args.emergency_price_factor*self.input_data.house_price[p] + self.input_data.supplier_region_distance[i][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price)*self.fk_ij[i,j,p,t].x for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T) for p in range(args.P))
        emergency_acqurie0 = sum(self.fk_ij[i,j,0,t].x for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T))
        emergency_acqurie1 = sum(self.fk_ij[i,j,1,t].x for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T))
        emergency_acqurie2 = sum(self.fk_ij[i,j,2,t].x for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T))
...
class master():
    ...
    def run(self,args):
        ...
        result_name = args.result_path + "/" + str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIU_" + str(args.TIU) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + "_mis_" + str(int(args.mis_factor*10)) + "_un_" + str(int(args.unused_factor*10)) + ".txt"
        f = open(result_name,"w+")
