...
class single_stage_chance():
    def __init__(self, args):
        ...
        if(args.modular == 0):
            for i in range(args.I):
                for t in range(args.T):
                    self.model.addConstr(self.v[i,0,t] == 0)
        ...
        for w in range(args.W):
            for j in range(args.J):
                for p in range(args.P):
                    for t in range(self.first_end_time_point+1):
        ...
        for i in range(args.I):
            ...
            for t in range(args.T-1):
                self.model.addConstr(self.a[i,t+1] + quicksum(self.s[i,p,t+1-self.mid_supplier_prod_time[i][p]] for p in range(args.P) if t+1-self.mid_supplier_prod_time[i][p] >= 0) ==  quicksum(self.s[i,p,t] for p in range(args.P) if t+self.mid_supplier_prod_time[i][p] <= args.T-1) + self.a[i,t])
                # 3k
        ...
        for i in range(args.I):
            for p in range(args.P):
                ...
                # 3e
                for t in range(args.T-1):
                    if(t-self.mid_supplier_prod_time_TIU[i][p] >= 0):
                        self.model.addConstr(self.v[i,p,t+1] + quicksum(self.x[i,w,p,t+1] for w in range(args.W) if t+1+self.iw_t[i][w] <= args.T-1) ==  self.v[i,p,t] + self.s[i,p,t-self.mid_supplier_prod_time[i][p]])
                    else:
        ...
        for w in range(args.W): 
            for t in range(self.first_end_time_point):
                for p in range(args.P):
                    self.model.addConstr(self.m[w,p,t+1] == quicksum(self.x[i,w,p,t-self.iw_t[i][w]] for i in range(args.I) if t-self.iw_t[i][w] >= 0) + self.m[w,p,t])
        ...
        for w in range(args.W): 
            for t in range(self.first_end_time_point,args.T-1):
                for p in range(args.P):
                    for k in range(args.K):
                        self.model.addConstr(self.mk[k,w,p,t+1] + quicksum(self.fk_wj[k,w,j,p,t+1] for j in range(args.J) if t+1+self.wj_t[w][j] <= args.T-1) - self.mk[k,w,p,t] == quicksum(self.x[i,w,p,t-self.iw_t[i][w]] for i in range(args.I) if (t+self.first_end_time_point-self.iw_t[i][w]) >= 0))
        ...
        for w in range(args.W):
            for t in range(args.T):
                for k in range(args.K):
                    self.model.addConstr(sum(self.house_volumn_factor[p]*self.fk_wj[k,w,j,p,t] for p in range(args.P) for j in range(args.J)) <= self.mid_staging_area_flow[w]) 
        ...
        for j in range(args.J):
            for t in range(args.T):
                for k in range(args.K):
                    self.model.addConstr(quicksum(self.fk_wj[k,w,j,0,t] for w in range(args.W)) == quicksum(self.fk0_j[k,j,p,t+self.house_install_time[0]] for p in range(1,args.P) if (t + self.house_install_time[0]) <= args.T-1)) 
        ...
        for j in range(args.J):
            for t in range(args.T):
                for p in range(1,args.P):
                    for k in range(args.K):
                        if(t - self.house_install_time[0] >= self.first_end_time_point + 1):
                            self.model.addConstr(1/self.house_0_match[p]*self.fk0_j[k,j,p,t-self.house_install_time[0]] + quicksum(self.fk_wj[k,w,j,p,t-self.wj_t[w][j]] for w in range(args.W) if (t-self.wj_t[w][j] >= self.first_end_time_point + 1))  == quicksum(self.yk[k,j,p,g,t] for g in range(args.G))) 
                        else:
                            self.model.addConstr(quicksum(self.fk_wj[k,w,j,p,t-self.wj_t[w][j]] for w in range(args.W) if (t-self.wj_t[w][j] >= self.first_end_time_point + 1)) == quicksum(self.yk[k,j,p,g,t] for g in range(args.G))) 
        ...
        for j in range(args.J):
            for t in range(args.T-1):
                for g in range(args.G):
                    for k in range(args.K):
                        self.model.addConstr(self.dk[k,j,g,t+1] == self.dk[k,j,g,t] + quicksum(self.yk[k,j,p,g,t+1] for p in range(1,args.P)))

        # demand limitation
        for j in range(args.J):
            for k in range(args.K):
                for g in range(args.G):
                    self.model.addConstr(self.dk[k,j,g,args.T-1] >= self.demand[k][g][j]*self.z[k])
